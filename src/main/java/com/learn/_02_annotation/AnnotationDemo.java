...
/**
 * 注解
 *  - 作用：标记在类的成分（类、成员变量、成员方法、构造方法、方法参数）上，对成分进行编译约束、标记等操作。
 *  - 注解相当于一种标记，是类的组成部分，可以给类的成分携带一些额外的信息。
 *  - 注解是给编译器或 JVM 使用的，编译器 JVM 可以根据注解来完成对应的功能。
 *
 *  编译器约束示例：
 *   - 方法重写约束：@Override
 *   - 函数式接口约束（约束接口类只能声明一个方法）：@FunctionalInterface
 */
 ...
 *    使用注解指定 value 属性值时可以不写 value=。
 *  - 如果注解有多个属性，且有属性没有配置默认值，则 value= 不可省略。
 */
